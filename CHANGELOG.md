...
- 新增优化器规则建议、字段差异预览、安全创建 Revision、审核发布、重新扫描验证及 7/28 天效果展示的真实 Playwright 后台旅程
- 新增 AI 深度分析任务创建、后台轮询和结构化结果展示的 Playwright 旅程，外部提供商响应使用确定性浏览器 mock
- 新增总览与报告聚合响应的 5 分钟版本化缓存，并由内容、问题、扫描、指标及 SEO 插件状态写入统一触发合并失效
...
- 修复内容资产和规划页快速翻页或切换筛选时，较早响应可能覆盖最新结果的问题
- 修复优化分析进行中切换内容时，较早响应可能回填到当前选择的问题
- 修复扫描失败后 Dashboard 自动刷新会清除后端失败原因的问题
- 修复 GSC/Bing 连接验证期间仍可断开，导致较早验证结果污染断开状态的问题
- 修复内容日历复核写入与刷新期间仍可重复复核或手动刷新，造成请求交错的问题
- 修复 AI 状态使用固定间隔轮询时慢请求相互重叠，增加负载并可能乱序更新的问题
- 修复优化闭环刷新期间切换内容后，旧修订面板响应可能恢复先前内容的问题
- 修复创建 Revision、刷新闭环状态与重新扫描验证可并发执行，导致较早响应覆盖新工作流状态的问题
- 修复 GSC/Bing 操作后的状态刷新会卸载 AI 设置表单并丢失尚未保存字段的问题
...
- Onboarding 表单添加字段级实时验证和错误提示

### Changed
- 调试日志统一使用 `Logger` 类，支持结构化上下文
- 优化前端构建产物（84KB JS + 9.73KB CSS）
- 改进空列表显示，提供清晰的操作引导
- 表单验证在用户输入时提供即时反馈
...
- 新增执行总结 `UPGRADE-SUMMARY.md` 和 `UI-UX-OPTIMIZATION-SUMMARY.md`
